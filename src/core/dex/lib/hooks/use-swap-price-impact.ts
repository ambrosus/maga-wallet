...
import { getAmountsIn, getAmountsOut } from '@core/dex/lib/contracts';
import { FIELD, SwapToken } from '@core/dex/types';
import {
      ...
      if (!hasWrapNativeToken) {
        ...
        if (pool && pool.pairAddress) {
          ...
          if (reserveIn && reserveOut) {
            ...
            return Number(Math.abs(Number(impact)).toFixed(ETH_DECIMALS));
          }
        }
      }
